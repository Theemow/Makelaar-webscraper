...
class MailService:
    ...
    def send_new_properties_email(self, recipients=None, nieuwe_properties=None):
        ...
        try:
            ...
            msg["To"] = ", ".join(recipients)  # Meerdere ontvangers scheiden met komma's
            msg["Subject"] = f"Nieuwe Huurwoningen - {datetime.now().strftime('%d-%m-%Y %H:%M')}"
            ...
            logger.info(f"E-mail succesvol verzonden naar {len(recipients)} ontvanger(s).")
            return True
    ...
    def _group_properties_by_broker(self, properties):
        ...
        for prop in properties:
            ...
            broker_name = prop.get('broker_naam', prop.get('makelaar_naam', 'Onbekende makelaar'))
    ...
    def send_error_email(self, recipients=None, error_message=""):
        ...
        try:
            ...
            msg["From"] = self.sender_email
            msg["To"] = ", ".join(recipients)  # Meerdere ontvangers scheiden met komma's
            msg["Subject"] = f"Fout in HuurhuisWebscraper - {datetime.now().strftime('%d-%m-%Y %H:%M')}"
            ...
            logger.info(f"Foutmelding e-mail verzonden naar {len(recipients)} ontvanger(s).")
            return True
