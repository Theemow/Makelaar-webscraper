...
class LogService:
    ...
    def log_app_start(self) -> None:
        ...
        self.root_logger.info(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    def log_app_end(self, new_properties_count: int, removed_properties_count: int) -> None:
        """
        ...
        self.root_logger.info(f"End time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def log_email_sent(self, success: bool, recipients: List[str]) -> None:
        ...
        if success:
            ...
            self.root_logger.info(f"Email notification successfully sent to: {recipient_str}")
        else:
            self.root_logger.error("Failed to send email notification")
    
    def log_broker_processing(self, broker_name: str, new_count: int, removed_count: int) -> None:
        """
        ...
        """
        self.root_logger.info(f"Broker: {broker_name} | New properties: {new_count} | Removed properties: {removed_count}")
