...
class DittersScraper(BaseScraper):
    ...
    def get_property_listings(self, page_num: int = 1) -> List[Dict[str, str]]:
        ...
        property_items = soup.select('div.aanbod-list__inner.product-starters-template-row-link')
        ...
        logger.info("Found %d properties on Ditters website (page %d)", len(property_items), page_num)
    ...
    def get_property_details(self, property_url: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Extract price
            price_element = soup.select_one('span.price, div.price span, div.UILabelPrice span')
            if price_element:
                details['huurprijs'] = self.clean_text(price_element.text)
            
            # Extract area - look for elements containing m²
            for element in soup.select('div.metadata-item span, div.specifications div, div.kenmerk'):
                text = self.clean_text(element.text)
                if 'm²' in text or 'm2' in text:
                    # Check if it's specifically about living area
                    if 'woonoppervlakte' in text.lower() or 'oppervlakte' in text.lower():
