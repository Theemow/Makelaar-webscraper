...
class ZonnenbergScraper(BaseScraper):
    ...
    def get_property_listings(self, page_num: int = 1) -> List[Dict[str, str]]:
        ...
        """
        # Check if there's pagination on the site
        if page_num > 1:
            url = f"{self.base_url}/woningaanbod/huur/page/{page_num}/"
        else:
        ...
        if response.status_code != 200:
            logger.error("Failed to get content from %s, status code: %s", url, response.status_code)
            return []
        ...
        for item in property_items:
            try:
                ...
                if not property_url or not property_url.startswith('http'):
                    ...
                    if property_id:
                        property_url = f"{self.base_url}/woningaanbod/huur/{property_id}/"
                ...
                # Skip invalid URLs
                if not property_url or '/page/' in property_url or property_url.endswith('/woningaanbod/huur/'):
                    continue
                
                # Extract the basic properties directly from the article
                # Address
                address_element = item.select_one('h4')
                address = self.clean_text(address_element.text if address_element else "N/A")
    ...
    def get_property_details(self, property_url: str) -> Dict[str, str]:
        ...
        try:
            # Get address
            address_element = soup.select_one('h1.property-title, h1, .object-title, .address, .property-address')
            if address_element:
            ...
            if location_element:
                ...
                # Strip postcode (4 cijfers gevolgd door 2 letters)
                details['naam_dorp_stad'] = re.sub(r'^\d{4}\s*[A-Z]{2}\s*', '', location_text)
            ...
            if details['naam_dorp_stad'] == "N/A":
                ...
                if meta_location and meta_location.get('content'):
                    details['naam_dorp_stad'] = self.clean_text(meta_location.get('content'))
                else:
                    # Zoek in de adrestekst
                    location_match = re.search(r'\b\d{4}\s*[A-Z]{2}\b\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)', details['adres'])
            ...
            if details['oppervlakte'] == "N/A":
                ...
                for element in soup.find_all(['span', 'div']):
                    ...
                    if 'm²' in text or 'm2' in text or '㎡' in text:
                        ...
                        if not ('€' in text or 'euro' in text or 'EUR' in text or 'prijs' in text.lower()):
