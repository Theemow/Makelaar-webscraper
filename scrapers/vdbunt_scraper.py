...
class VdBuntScraper(BaseScraper):
    ...
    def get_property_listings(self, page_num: int = 1) -> List[Dict[str, str]]:
        ...
        for item in property_items:
            try:
                ...
                address = self.clean_text(address_element.text if address_element else None)
                ...
                price_element = item.select_one('span.kenmerk.huurprijs span.kenmerkValue')
                ...
                area_element = item.select_one('span.kenmerk.woonoppervlakte span.kenmerkValue')
    ...
    def get_property_details(self, property_url: str) -> Dict[str, str]:
        ...
        try:
            ...
            details['adres'] = self.clean_text(address_element.text if address_element else None)
            
            # Location
            location_element = soup.select_one('span.locality')
            details['naam_dorp_stad'] = self.clean_text(location_element.text if location_element else None)
            
            # Rental price - we adjust the selector for the detail page
            price_element = soup.select_one('.kenmerk.huurprijs .kenmerkValue, .obj-kenmerken.first .huurprijs .rechts')
            details['huurprijs'] = self.clean_text(price_element.text if price_element else None)
            
            # Surface area - we adjust the selector for the detail page
            details['oppervlakte'] = "N/A"  # Default value
            
            area_element = soup.select_one('.kenmerk.woonoppervlakte .kenmerkValue, .obj-kenmerken .woonoppervlakte .rechts')
            if area_element:
    ...
    def get_all_listings(self, max_pages: int = 5) -> List[Dict[str, str]]:
        ...
        """
        logger.info("VdBunt scraper only processes first page since pagination is not supported")
