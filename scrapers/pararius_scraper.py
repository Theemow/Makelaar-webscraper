    ...
    def get_property_listings(self, page_num: int = 1) -> List[Dict[str, str]]:
        ...
        for listing in listings:
            try:
                ...
                if city_elem:
                    property_data['naam_dorp_stad'] = self.clean_city_name(city_elem.text)
                ...
                area_elem = listing.select_one('.listing-search-item__features .surface')
                if not area_elem:
                    area_elem = listing.select_one('.illustrated-features__item--surface-area')
                ...
                if link_elem and link_elem.get('href'):
                    property_data['link'] = urljoin('https://www.pararius.nl', link_elem.get('href'))
        ...
        try:
            ...
            for feature in features:
                if 'Woonoppervlakte' in feature.text:
                    details['oppervlakte'] = self.clean_text(feature.text.replace('Woonoppervlakte', ''))
                    break
